...
% newelm()
siec = newelm(zakres,[liczba_n_h1 liczba_n_o],{'tansig','purelin'},'trainlm');
siec.trainParam.epochs=100;
siec.trainParam.goal=0;
...
title(['1, siec jednokierunkowa, newff, MSE = ', num2str(mse_newff)]);
...
plot(y,'b--');
legend('matching', 'input')
title(['1, siec jednokierunkowa, newff, MSE = ', num2str(mse_newelm)]);
...
plot(y,'b--');
legend('matching', 'input')
title(['1, siec jednokierunkowa, newff, MSE = ', num2str(mse_newrb)]);
...
plot(y,'b--');
legend('matching', 'input')
title(['1, siec jednokierunkowa, newff, MSE = ', num2str(mse_newrbe)]);
